...
def two_sum(nums, target):
    ...
    This implementation uses the 'in' statement for iterable object and
    the index method for list object.
...
def two_sum_by_hashmap(nums, target):
    """Find the indices of the two numbers in a list that add up to the target
    This implementation uses the dict object as hashmap to reduce
    the query time.
...
if __name__ == "__main__":
    ...
    t1 = Timer(
        "%s(nums, target)" % func_name,
        "from __main__ import %s; nums=%s; target=%s" % (func_name, nums, target),
    )
    print(
        "Executing func %s %d times costs %.3f second!"
        % (func_name, exe_num, t1.timeit(number=exe_num))
    )
